 ...
 * account position relative to the parent node.  In order to get the full screen
 * */
...
export default class ElementPosition {
	...
	// Builds a MouseTypes.Position object based on the top-left coordinates of the
	// parent node.
	private static buildRelativePosition(
		...
		return {topLeft, topRight, bottomLeft, bottomRight} as MouseTypes.Position;
	}

	private static applyParentOffset(
		childNode: YogaNode,
		child: MouseTypes.Position,
		parent: MouseTypes.Position,
	): MouseTypes.Position {
	...
	public static getScreenPosition(childNode: YogaNode): MouseTypes.Position {
